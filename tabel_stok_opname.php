...
// HANDLE DELETE via POST (no separate file)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_id'])) {
    $del_id = intval($_POST['delete_id']);
    // delete stok_opname (detail ON DELETE CASCADE will remove detail)
...
.btn-delete{background:#ff7b7b; color:#fff}
.btn-export{background:#2e86de;color:#fff}
.pager a{margin-right:6px}
.notice{background:#e6ffea; padding:10px; border:1px solid #bde6c6; margin-bottom:8px}
</style>
</head>
<body>
<div class="container">
    ...
    <div style="margin-bottom:10px;">
        ...
        <a href="export_pdf_stok_opname.php" target="_blank"><button class="btn btn-export">Export Semua ke PDF</button></a>
    </div>
        ...
        <tbody>
            ...
            <?php else: foreach($rows as $r): ?>
                <tr>
                    ...
                    <td><?php echo 'Rp. '.number_format($r['subtotal_fisik'],0,',','.'); ?></td>
                    <td><?php echo 'Rp. '.number_format($r['fisik_total'],0,',','.'); ?></td>
                    <td><?php echo 'Rp. '.number_format($r['saldo_sistem'],0,',','.'); ?></td>
                    <td><?php echo 'Rp. '.number_format($r['selisih'],0,',','.'); ?></td>
                    <td><?php echo date('d-M-Y H:i', strtotime($r['tanggal_opname'])); ?></td>
                    <td class="actions">
                        <a href="stok_opname.php?edit=<?php echo intval($r['id']); ?>"><button class="btn btn-edit">Edit</button></a>
                        ...
                        <a href="export_pdf_stok_opname.php?id=<?php echo intval($r['id']); ?>" target="_blank">
